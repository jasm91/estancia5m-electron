  ...
  try {
    ...
    if (type === 'diesel' && data.type === 'recarga') {
      try {
        ...
        const tank = data.tank_id ? (tanks.find(t => t.id === data.tank_id) || tanks[0]) : tanks[0];
...
app.post('/api/sync-push', auth, async (req, res) => {
  try {
    ...
    setImmediate(async () => {
      try {
        const allTables = ['lots','vet_products','treatments','health_alerts','sales','purchases','employees','field_activities','tasks','pesajes','advances','maintenance','agua','sal','conteo','partos','alimento','animals','animal_movements','lluvias','diesel','aceite','cuentas','kardex','historial_sueldos','compras_ganado','bajas'];
...
app.post('/api/cloud-backup', auth, async (req, res) => {
  try {
    const tenantId = req.tenantId;
    const tables = ['lots','vet_products','treatments','health_alerts','sales','purchases','employees','field_activities','tasks','pesajes','advances','maintenance','agua','sal','conteo','partos','alimento','animals','animal_movements','lluvias','diesel','aceite','cuentas','kardex','historial_sueldos','compras_ganado','bajas'];
    const snapshot = { timestamp: new Date().toISOString(), tenant: req.tenant.name, tables: {} };
